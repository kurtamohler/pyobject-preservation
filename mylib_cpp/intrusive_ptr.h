// This is copied from PyTorch and modified
...
//template <typename T>
//struct MaybeOwnedTraits<mylib_cpp::intrusive_ptr<T>> {
//  using owned_type = mylib_cpp::intrusive_ptr<T>;
//  using borrow_type = mylib_cpp::intrusive_ptr<T>;
//
...
inline void decref(intrusive_ptr_target* self) {
  // Let it die
  mylib_cpp::intrusive_ptr<intrusive_ptr_target>::reclaim(self);
  // NB: Caller still has 'self' pointer, but it's now invalid.
  // If you want more safety, used the actual mylib_cpp::intrusive_ptr class
}

template <typename T>
inline T* make_weak(T* self) {
  // NB: 'this' is a strong pointer, but we return a weak pointer
  auto ptr = mylib_cpp::intrusive_ptr<T>::reclaim(self);
  mylib_cpp::weak_intrusive_ptr<T> wptr(ptr);
  ptr.release();
  return wptr.release();
}

inline size_t use_count(intrusive_ptr_target* self) {
  auto ptr = mylib_cpp::intrusive_ptr<intrusive_ptr_target>::reclaim(self);
...
inline void decref(weak_intrusive_ptr_target* self) {
  // Let it die
  mylib_cpp::weak_intrusive_ptr<intrusive_ptr_target>::reclaim(self);
  // NB: You still "have" the 'self' pointer, but it's now invalid.
  // If you want more safety, used the actual mylib_cpp::weak_intrusive_ptr class
}

template <typename T>
inline T* lock(T* self) {
  auto wptr = mylib_cpp::weak_intrusive_ptr<T>::reclaim(self);
  auto ptr = wptr.lock();
  wptr.release();
  return ptr.release();
}
...
inline size_t use_count(weak_intrusive_ptr_target* self) {
  auto wptr = mylib_cpp::weak_intrusive_ptr<intrusive_ptr_target>::reclaim(self);
